...
function RouteComponent() {
	...
	const { ref: leadsRef, inView: leadsInView } = useInView({
		triggerOnce: true,
		threshold: 0.1,
	});
	...
	return (
		<motion.div initial="hidden" animate="visible" variants={pageVariants}>
			...
			{/* Principal and faculty advisory Section */}
			<section ref={facultyRef} className="py-16 px-4 sm:px-6 md:px-8 ">
				<div className="max-w-7xl mx-auto">
					<h2 className="text-4xl font-medium font-heading text-white text-center">
						Faculty
					</h2>
					<motion.div
						className="mt-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4  gap-8 justify-center"
						variants={gridContainerVariants}
						initial="hidden"
						animate={facultyInView ? "visible" : "hidden"}
					>
						{faculties.map((member, i) => (
							<MemberCard
								key={member.name}
								member={member}
			...
			<section ref={advisorRef} className="py-16 px-4 sm:px-6 md:px-8 ">
				<div className="max-w-7xl mx-auto">
					...
					<motion.div
						className="mt-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3  gap-8 justify-center"
						variants={gridContainerVariants}
						initial="hidden"
